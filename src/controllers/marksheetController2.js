...
const generateMarksheet = async (req, res) => {
    try {
        ...
        if (admin.adminId === adminId && admin.sessionToken === sessionToken) {
            ...
            let { universityName, marksheet_description, sr_number, st_number, subjects, passing_description, passing_criteria, marksheet_code, date_of_issue } = req.body;
...
const addSubjects = async (req, res) => {
    try {
        ...
        if (admin.adminId === adminId && admin.sessionToken === sessionToken) {
            ...
            let { code, subject_details, year, session, max_number, total_number } = req.body;
...
const deleteMarksheet = async (req, res) => {
    try {
        ...
        if (admin.adminId === adminId && admin.sessionToken === sessionToken) {
            ...
            let QRImageName = marksheet.qrCode_img.qrName;
            let imgPath = path.join(__dirname, "..", "QRImages", QRImageName);
...
const updateMarksheet = async (req, res) => {
    try {
        ...
        if ("qrCode_img" in e || (req.files && req.files.qrCode_img)) {
            ...
            if (!QRCode) {
                return res.status(400).send({ status: false, message: "No QR Code uploaded" });
            }

            let oldQRCodeName = m.qrCode_img.qrName;
            let oldQRCodePath = path.join(__dirname, "..", "QRImages", oldQRCodeName);
            if (fs.existsSync(oldQRCodePath)) {
                fs.unlinkSync(oldQRCodePath);
            }

            let currentIpAddress = getCurrentIPAddress();
            let imgRelativePath = "/QRImages/";
            let imgUniqName = uuid.v4() + "." + QRCode.name.split(".").pop();
            let imgUrl = `http://${currentIpAddress}:${port}${imgRelativePath}`;
            let imgSavingPath = path.join(__dirname, "..", "QRImages", imgUniqName);

            QRCode.mv(imgSavingPath, (err) => {
                if (err) throw err;
            });
