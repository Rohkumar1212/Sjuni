...
const { saveFile } = require("../uitls/fileSave");
...
const createAdmin = async (req, res) => {
  try {
    ...
    if (!profilePic) {
      return res
        .status(400)
        .send({ status: false, message: "No profile pic uploaded" });
    }
...
const getAllConsultants = async (req, res) => {
  try {
    ...
    if (
      superAdmin.adminId === adminId &&
      superAdmin.sessionToken === sessionToken
    ) {
      ...
      let allConsultants = await adminModel.find({
        role: "ADMIN",
        isSuperAdmin: false,
      }); //.countDocuments();
  ...
  try {
    ...
    if (admin.adminId === adminId && admin.sessionToken === sessionToken) {
      ...
      if (!paymentQRCode) {
        return res
          .status(404)
          .send({ status: false, message: "Payment QR Code not found" });
      }
  ...
  try {
    ...
    if (admin.adminId === adminId && admin.sessionToken === sessionToken) {
      let user = await userModel
        .findOne({ userId })
        .populate([{ path: "marksheet" }, { path: "certificate" }]);
