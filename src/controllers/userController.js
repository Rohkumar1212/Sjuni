...
const addStudent = async (req, res) => {
    try {
        ...
        console.log( consultantId, sessionToken)
        ...
        if (consultant.adminId === consultantId && consultant.sessionToken === sessionToken) {
            let { name, roll_number, registration_number, father_name, mother_name, address, email, phone, date_of_birth, category, aadhar_number, pan_number, course_name } = req.body;

            if (!name || !roll_number || !registration_number || !father_name || !mother_name || !address || !email || !phone || !date_of_birth || !category || !aadhar_number || !pan_number || !course_name) {
                return res.status(400).send({ status: false, message: "All fields are required" });
            }
            ...
            if (!profilePic) {
                return res.status(400).send({ status: false, message: "Profile picture is not uploaded" });
            }

            let currentIpAddress = getCurrentIPAddress();
            let profilePicRelativePath = "/userImages/";
            let profilePicUniqName = uuid.v4() + "." + profilePic.name.split(".").pop();
            let profilePicFullPath = `http://${currentIpAddress}:${port}${profilePicRelativePath}`;
            let profilePicSavingPath = path.join(__dirname, "..", "userImages", profilePicUniqName);
...
const addEducation = async (req, res) => {
    try {
        ...
        if (!consultant) {
            return res.status(400).send({ status: false, message: "No consultant found with this consultant Id" });
        };
        ...
        if (sessionToken === consultant.sessionToken) {
            ...
            if (!marksheet) {
                return res.status(400).send({ status: false, message: "Marksheet is not uploaded" });
            };

            let currentIpAddress = getCurrentIPAddress();
            let marksheetPath = "/marksheetImages/";
            let marksheetUniqName = uuid.v4() + "." + marksheet.name.split(".").pop();
            let marksheetFullPath = `http://${currentIpAddress}:${port}${marksheetPath}`;
            let marksheetSavingPath = path.join(__dirname, "..", "marksheetImages", marksheetUniqName);

            marksheet.mv(marksheetSavingPath, (err) => {
                if (err) {
                    throw err;
                };
            });
            ...
            let marksheetObj = {
                fileName: marksheetUniqName,
                filePath: marksheetFullPath,
            ...
            let isPresent = user.education.some(item => item.student_class === educationObj.student_class);
    ...
    try {
        ...
        if (consultant.adminId === consultantId && consultant.sessionToken === sessionToken) {
            ...
            if (!isValidObjectId(eduId)) {
                return res.status(400).send({ status: false, message: "Invalid eduId" });
            };
            ...
            for (let edu of user.education) {
                if (edu._id.toString() === eduId) {
                    ...
                    if ("marksheet" in reqBody || (req.files && req.files.marksheet)) {
                        ...
                        if (!marksheet) {
                            return res.status(400).send({ status: false, message: "No marksheet uploaded" });
                        }

                        let currentIpAddress = getCurrentIPAddress();
                        let marksheetRelativePath = "/marksheetImages/";
                        let marksheetUniqName = uuid.v4() + "." + marksheet.name.split(".").pop();
                        let marksheetFullPath = `http://${currentIpAddress}:${port}${marksheetRelativePath}`;

                        let marksheetSavingPath = path.join(__dirname, "..", "marksheetImages", marksheetUniqName);

                        let oldMarksheet = edu.marksheet.fileName;
                        let oldMarksheetPath = path.join(__dirname, "..", "marksheetImages", oldMarksheet);

                        fs.unlinkSync(oldMarksheetPath);
...
const deleteStudent = async (req, res) => {
    try {
        ...
        if (consultant.adminId === consultantId && consultant.sessionToken === sessionToken) {
            ...
            let userProfileName = user.profilePic.picName;
            let imgPath = path.join(__dirname, "..", "userImages", userProfileName);
            ...
            if (user.education.length) {
                for ( let edu of user.education) {
                    ...
                    let imgPath = path.join(__dirname, "..", "marksheetImages", eduImgName);
...
const getMarksheetByStudent = async (req, res) => {
    try {
        ...
        let student = await userModel.findOne({ roll_number:rollNo, registration_number:regNo});

        if (!student) {
            return res.status(404).send({ status: false, message: "Student Not Found"});
        }; 

        let studentMarksheet = await marksheetModel.findOne({ roll_number:rollNo, registration_number:regNo });

        if (!studentMarksheet) {
            return res.status(404).send({ status: false, message: "No marksheet found" });
        };
